...
class Broker:
    def __init__(self, socket_path, deadline, *, catalog=None):
        ...
        self.calls = {"deepline": 0, "openrouter": 0}
    ...
    def request(self, operation, parameters):
        provider = operation.split(".")[0]
        if operation not in {"deepline.execute", "openrouter.chat"}:
        ...
        with self.lock:
            ...
            if provider == "deepline" and self.provider_blocked:
                raise BrokerRefusal("provider_blocked_after_uncertain_call")
            if self.calls[provider] >= {"deepline": 30, "openrouter": 60}[provider]:
        ...
        frame = json.dumps({"schema_version": "leadpoet.lab_arena.operation_frame.v1",
            ...
            "timeout_ms": max(1, min(int(remaining * 1000), 60000 if provider == "deepline" else 120000))},
